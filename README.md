...
- 이미지 캐러셀 : 순수 자바스크립트 사용해서 구현
- 브랜드 정보 콘텐츠 : JS로 HTML 요소에 클래스명을 추가, 삭제를 사용 => 리스트 브랜드 별로 정렬 구현
- CSS3 속성 사용으로 크롬브라우져와 최신 브라우져에 최적화되어 있습니다. 
- IR기법 (image-replace) : 리더기는 텍스트를 읽어주되 시각적으론 이미지로 대체
- Gulp 사용 : Sass → CSS로 컴파일 후 병합, Javascript 파일 병합
...
- [x] 캐러셀 콘텐츠 만들기 : 2017/01/24
- [ ] 캐러셀 콘텐츠 만들기 디테일 : 4번이미지에서 1번이미지로 너어가는 순간 슬라이딩 자연스럽게..
- [ ] 캐러셀 콘텐츠 만들기 디테일 : 이미지 어두우면 버튼영역 흰색으로 변경
- [x] 페이지 하단영역
- [X] 핫 아이템 콘텐츠 : 데스크탑 버전일때 레이아웃 잡기 : 2017/01/23
- [X] 핫 아이템 콘텐츠 : 영화 콘텐츠 부분 상세 콘텐츠 내용 채우고 스타일링 : 2017/01/24
- [X] 핫 아이템 콘텐츠 : 반응형 대응 하기 : 2017/01/24
- [ ] 핫 아이템 콘텐츠 디테일 : 배경 부분 원들 움직임
- [ ] 인포 콘텐츠  : 마우스 호버시 애니메이션 같은 동작 구현하기
- [x] 브랜드 콘텐츠 : 콘텐츠 채우기, 메뉴(브랜드 파트) 클릭시 해당 브랜드 별로 정렬 구현 : 2017/01/17
- [x] 디자인 구조 바꾸기 : 모바일, 태블릿, 데스크탑 반응형으로 만들기 위해 디자인 구조를 좀 바꿔야할듯
...
- [ ] 탑버튼 디테일 : 클릭시 애니메이션 사용해서 부드럽게 올라가기
...
- 이미지캐러셀
  + 자동슬라이딩
  + 이미지의 텍스트는 진짜 텍스트 이미지 아님.
  + 이전, 다음 버튼과 슬라이딩 제어 버튼이 있어 슬라이딩 컨트롤 가능
  + 모바일 데탑은 이미지캐러셀, 모바일은 정적인 콘텐츠
